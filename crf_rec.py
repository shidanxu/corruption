...
def mytrain(mode=None, nn=None, ntest=1):
    ...
    if mode=="test":
        tagger.open(testfname)
    else:
        tagger.open('output.crfModel')

    info = tagger.info()
    print 'info:',info.transitions
    # exit(0)
...
def word2features(preword, word, wordnext):
    features = ["regex_punish="+feature1(preword, word, wordnext), "regex_crime="+feature2(preword, word, wordnext), "regex_amount="+feature3(preword, word, wordnext),]
...
def feature5(preword_tuple, word_tuple, nextword_tuple):
    ...
    # if re.search(POSITION_REGEX, txt):
